...
def convert(data_type: str, input_dir: str, data_dir: str) -> None:
    ...
    for path in tqdm(dirs, desc=data_type, ncols=78):
        # path => ./data//train/1000216489776414077
        ...
        bands = []
        for k in [11, 14, 15]:
            # '%s/band_%02d.npy' % (path, k) => ./data//train/1000216489776414077/band_11.npy            
            # np.load('%s/band_%02d.npy' % (path, k)).shape => (256, 256, 8)
        ...
        if data_type == 'train':
            ...
            # annot.shape => (256, 256, 1, 4)            
            annot = np.mean(annot, axis=3)
            # annot.shape => (256, 256, 1)
...
def create_df(data_type: str, data_dir: str) -> None:
    ...
    # 3   100071707854144929   ./data/compact4/train/100071707854144929.h5          0
    # 4  1000823728928031783  ./data/compact4/train/1000823728928031783.h5        369    
