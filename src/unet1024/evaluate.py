...
# telling the Python program that when it receives a Ctrl+C (SIGINT) signal, it should behave as it normally does by default, which is to terminate the program.
signal.signal(signal.SIGINT, signal.SIG_DFL)

# set the number of CPU threads to be used for parallel computation to 1.
...
# nfolds => 10
# cfg['kfold']['folds'] => 3,4
folds = util.as_list(cfg['kfold']['folds'])  
# folds => [3, 4]
...
# data.df.head(3) =>
#                file_id                                      filename  label_sum
# 0  1000216489776414077  ./data/compact4/train/1000216489776414077.h5          0
# 1  1000603527582775543  ./data/compact4/train/1000603527582775543.h5       5579
# 2  1000660467359258186  ./data/compact4/train/1000660467359258186.h5        674
...
for ifold, (idx_train, idx_val) in enumerate(kfold.split(data.df)):
    ...
    loader_val = data.loader(idx_val, cfg) # comment my_
    loader_test = data_test.loader(None, cfg) # comment my_
